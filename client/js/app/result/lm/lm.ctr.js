...
function getMean(arr){
  ...
  var regCount = jsonReturn.coords.length; // print this
  var regMeanX = getMean(plotX); // print this
  var regMeanY = getMean(plotY); // print this
  
  // create formula string
  var regRoundInt = Number((jsonReturn.intercept).toFixed(2));
  var regRoundIntAbs = Math.abs(regRoundInt);
  var regRoundSlope = Number((jsonReturn.const).toFixed(2))
  if(regRoundInt >= 0) {var addSub = " + "} else {var addSub = " - "};
  var formulaString = "y = " + regRoundSlope + "x" + addSub + regRoundIntAbs; // print this
